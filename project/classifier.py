...
import numpy as np
from A.util import GaussianFourierProjection, Dense, set_all_seeds
...
if __name__ == "__main__":
    ...
    dataloader_test = torch.utils.data.DataLoader(
        datasets.MNIST('./mnist_data', download=True, train=False, transform=test_transform),
        batch_size=batch_size,
        shuffle=False
    )
    ...
    for epoch in range(epochs):
        ...
        for i, (x, c) in enumerate(dataloader_train):
            ...
            # Update progress bar
            progress_bar.set_postfix(loss=f"{loss.item():.4f}", epoch=f"{epoch + 1}/{epochs}", lr=f"{scheduler.get_last_lr()[0]:.2E}")
            progress_bar.update()
        ...
        if test_accuracy > best_accuracy:
            ...
            print(f"New best accuracy: {best_accuracy:.4f}. Model saved to {save_path}.")
